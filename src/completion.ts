...
export async function getAllSymbolsOfType<T extends Symbol>(
  scope: ScopedSymbol,
  type: new (...args: any[]) => T
): Promise<T[]> {
  ...
  if (parent) {
    const subSymbols = await getAllSymbolsOfType(parent as ScopedSymbol, type);
...
export async function getSuggestionsForParseTree(
  ...
  ignored.push(
    KotlinParser.QUOTE_OPEN,
    KotlinParser.QUOTE_CLOSE,
    KotlinParser.TRIPLE_QUOTE_OPEN
  );
  ...
  if (
    ...
    const vars = await suggestVariables(symbolTableFn(), position);
